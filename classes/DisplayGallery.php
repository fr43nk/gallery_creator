...
abstract class DisplayGallery extends \Module
{
	...
	protected function getAlbumTemplateVars($intAlbumId, $strContentType)
	{
		...
		if(TL_MODE == 'FE')
		{
			$objPage->description .= (($objPage->description != '') ? ', ' : '') . str_replace(',,', ',', implode(', ', array(
					specialchars($objAlbum->name), specialchars($objAlbum->event_location)
				)));
			$GLOBALS['TL_KEYWORDS'] .= (($GLOBALS['TL_KEYWORDS'] != '') ? ', ' : '') . str_replace(',,', ',', implode(', ', array(
					specialchars($objAlbum->name), specialchars($objAlbum->event_location)
				)));
		}
